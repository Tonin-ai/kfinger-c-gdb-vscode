...
typedef struct hash_dict
{
	...
	unsigned int size;	 // numero di entry del dizionario

} hash_dict;

void initHashDict(hash_dict *hdict, unsigned int size);
...
{
	if (!*dict)
	{ // controlla se la entry di hdict relativa al dizionario dict è ha già della memoria allocata
